...
class TelegramAgent:
    def __init__(self, mcp_client, memory):
        ...
        async def get_currency(currency_code: str) -> str:
            """Получить курс валюты. Аргумент — код валюты, например USD, EUR, BTC, RUB. Используй когда спрашивают о курсе валют."""
            return await self.mcp_client.get_currency(currency_code)
        
        @tool
        async def web_search(query: str) -> str:
            """Поиск актуальной информации, новостей или ответов на вопросы в интернете. Используй для любых актуальных данных или когда не знаешь ответ."""
        ...
        # Системный промпт
        self.system_prompt = (
            "Ты — полезный AI-ассистент в Telegram. Отвечай на русском языке. "
            "Если пользователь спрашивает о погоде — ОБЯЗАТЕЛЬНО вызови get_weather. "
            "Если о курсе валют — вызови get_currency. "
            "Если нужна актуальная информация или ты не уверен в ответе — вызови web_search. "
            "Если пользователь называет личные данные о себе (имя, предпочтения, город) — вызови save_user_fact. "
            "Никогда не выдумывай данные — всегда используй инструменты для фактов. "
    ...
    async def _run_tool(self, tool_name: str, tool_args: dict) -> str:
        ...
        try:
            # LangChain tools возвращают корутину для async функций
            result = await tool_fn.ainvoke(tool_args)
            return str(result)
        except Exception as e:
            logger.error(f"Ошибка инструмента {tool_name}: {e}")
            return f"⚠️ Ошибка при вызове {tool_name}: {str(e)}"
    ...
    async def process_message(
        ...
        try:
            ...
            # Добавляем историю чата (конвертируем из LangChain формата)
            if chat_history:
            ...
            for iteration in range(5):
                logger.info(f"🔄 Итерация LLM {iteration + 1}/5")
                ...
                for tc in response.tool_calls:
                    ...
                    logger.info(f"🔧 Вызов инструмента: {tool_name}({tool_args})")
            ...
            return final_msg.content if hasattr(final_msg, "content") else "⚠️ Не удалось получить ответ."
